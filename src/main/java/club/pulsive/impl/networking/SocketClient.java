...
public class SocketClient extends WebSocketClient {
    ...
    public void onMessage(String message) {
        if (gson == null || packetHandler == null || message == null) return;
        String decoded;
        try {
            decoded = PacketEncoder.decode(message);
        } catch (Exception e) {
            return;
        }
        Packet packet = gson.fromJson(decoded, Packet.class);
        if (packet != null) {
    ...
    public void onError(Exception ex) {
        if (ex != null) {
            Logger.print("WebSocket error: " + ex.getMessage());
        }
    }
}

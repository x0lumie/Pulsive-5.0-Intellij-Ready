...
public class CServerCommandPacket extends Packet {
    ...
    public CServerCommandPacket(final CommandOperation commandOperation, Object request, String tag) {
        super(getIdForPacket(CServerCommandPacket.class));
        ...
        switch (commandOperation) {
            ...
            case PACKET: {
                ...
                if (client != null && client.getGson() != null) {
                    data.addProperty("request", Base64.getEncoder().encodeToString(client.getGson().toJson(request, Packet.class).getBytes()));
                }
                break;
            }
        }
    ...
    public Object getRequest() {
        if (data.get("operation") == null || data.get("request") == null) return null;
        switch (CommandOperation.valueOf(data.get("operation").getAsString())) {
            case PACKET: {
                SocketClient client = getSocketClient();
                if (client == null || client.getGson() == null) return null;
                return client.getGson().fromJson(new String(Base64.getDecoder().decode(data.get("request").getAsString())), Packet.class);
            }
            default: {
